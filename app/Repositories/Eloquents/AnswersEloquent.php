...
use App\QuestionAnswersTranslation;
use App\Blog;
use App\Repositories\Interfaces\Repository;
use Illuminate\Support\Facades\Config;

class AnswersEloquent implements Repository
{

    private $model, $translation;

    public function __construct(QuestionsAnswer $model, QuestionAnswersTranslation $translation)
    {
        $this->model = $model;
    ...
    function anyData()
    {
        ...
        return datatables()->of($data)
            ->addColumn('answer', function ($item) {
                ...
                return '
                                    <a href="' . url(admin_web_url() . '/answers-edit/' . $item->id) . '" class="btn btn-circle btn-icon-only purple edit">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    <a href="' . url(admin_web_url() . '/answers/' . $item->id) . '" class="btn btn-circle btn-icon-only red delete">
    ...
    function create(array $attributes)
    {
        ...
        $model = QuestionsAnswer::create(['question_id'=>$attributes['question_id']]);
        if ($model) {
    ...
    function update(array $attributes, $id = null)
    {
        ...
        if (isset($model)) {
            if(isset($attributes['question_id'])){
                $model->question_id = $attributes['question_id'];
                $model->save();
            }
