var answers_tbl = $("#answers_tbl");
$(document).ready(function () {


    if ($("#answers_tbl").length) {

        answers_tbl.on('preXhr.dt', function (e, settings, data) {
            // data.username = $('#username_c').val();
        }).dataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                url: baseURL + "/web/answers-data",
                "dataSrc": function (json) {
                    //Make your callback here.
                    if (json.status !== undefined && !json.status) {
                        $('#answers_tbl_processing').hide();
                        location.reload();
                        //
                    } else
                        return json.data;
